...
class ComehomePoseTF2():
    ...
    Handels the navigation goals given by the operator via rviz. Should the given position not
    oiginate from the map frame (current frame in rviz) it will transforms the given position into
    ...
    def __init__(self):
        ...
        self.pub_new_goal = rospy.Publisher('/move_base_simple/goal', PoseStamped, queue_size=1)

        self.sub_goal = rospy.Subscriber('/rviz/goal', PoseStamped, self.goal_callback)
        self.sub_home = rospy.Subscriber('/home_goal', PoseWithCovarianceStamped, self.home_callback)

        self.srv_home = rospy.Service('/Comehome_GoalPoseTF/drive_home', Trigger, self.home_server)
    ...
    def goal_callback(self, msg):
        ...
        if source_frame != msg.header.frame_id:
            rospy.logerr("Source frame set to {}, but selected frame is {}".format(source_frame, msg.header.frame_id))
            rospy.logerr("Please select the %s frame", source_frame)
            return

        try:
            rospy.loginfo("Transforming pose from {} to {}".format(source_frame, map_frame))
            trans = self.tf_buffer.lookup_transform(map_frame, source_frame, look_up_time,
                                                    rospy.Duration(self.config.timeout))

        except tf2.LookupException as ex:
            rospy.logwarn_throttle(3, str(look_up_time.to_sec()))
            rospy.logwarn_throttle(3, ex)
            rospy.logwarn_throttle(3, "Could not get transformation from {} to {}".format(source_frame, map_frame))
            return
