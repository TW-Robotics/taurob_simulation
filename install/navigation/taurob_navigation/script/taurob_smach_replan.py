...
class Planning(smach.StateMachine):
        ...
        with self:
            ...
            smach.StateMachine.add(
                ...
                transitions={
                    ...
                    'invalid' : 'invalid',
                    'preempted' : 'preempted',
                    'aborted' : 'aborted'})#ka warum ich aborted brauch
    ...
    def get_path_result_cb(userdata, status, result):
        ...
        if result.outcome == GetPathResult.SUCCESS:
            ...
            print 'Planning could not get any path %s:\n%s' % (str(result.outcome), result.message)
            return 'invalid'
        else:
            print 'Planning terminated with non-success status code %s:\n%s' % (str(result.outcome), result.message)
            return 'failure'
...
class Controlling(smach.StateMachine):
    ...
    def __init__(self):
        smach.StateMachine.__init__(
            self,
            outcomes=['succeeded', 'preempted', 'failure', 'aborted', 'change_controller'],
            input_keys=['path'],
            output_keys=['outcome', 'message', 'final_pose', 'dist_to_goal', 'angle_to_goal', 'recov_cont', 'switch_controller', 'second_controller']
        )
    ...
    @smach.cb_interface(
        ...
        output_keys=['path', 'outcome', 'message', 'final_pose', 'dist_to_goal', 'angle_to_goal', 'local_planer', 'recov_cont', 'switch_controller', 'second_controller'],
    ...
    def exe_path_result_cb(userdata, status, result):
        ...
        if result.outcome == ExePathResult.SUCCESS:
            ...
            return 'preempted'
        elif result.outcome in controller_failed_map and userdata.switch_controller is True:
...
class Replanning(smach.Concurrence):
    ...
    def __init__(self):
        smach.Concurrence.__init__(
            self,
            outcomes=['received_goal', 'succeeded', 'preempted', 'failure', 'invalid', 'aborted'],
            ...
            outcome_map={
                ...
                'preempted': {'WAIT_FOR_GOAL': 'preempted',
                              'PLANNING': 'preempted'
                              },
...
class DriveToGoal(smach.Concurrence):
    ...
    def __init__(self):
        smach.Concurrence.__init__(
            self,
            outcomes=['succeeded', 'preempted', 'received_path', 'failure', 'aborted', 'change_controller'],
            default_outcome='aborted',
            input_keys=['path'],
            output_keys=['target_pose', 'path'],
            outcome_map={
...
def main():
    ...
    sis = smach_ros.IntrospectionServer('mbf_state_machine_server', SM, '/MBF_SM_REPLANNING')
    sis.start()
